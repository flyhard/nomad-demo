...
job "fabio" {
  ...
  group "proxy" {
    ...
    task "fabio" {
      ...
      config {
        command = "/local/fabio"
        args    = [ "-ui.addr", ":${NOMAD_PORT_admin}", "-proxy.addr", ":${NOMAD_PORT_proxy}" ]
      }
      ...
      #
      artifact {
        source      = "http://github.com/fabiolb/fabio/releases/download/v1.5.2/fabio-1.5.2-go1.9-darwin_amd64"
        mode        = "file"
